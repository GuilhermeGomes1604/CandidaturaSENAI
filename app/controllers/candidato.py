...
def cadastrar_candidato():
    ...
    nome_social = request.form.get('nome-social', '').strip()
    ...
    periodo_estudo = request.form.get('periodo-estudo', '').strip()
...
# @verificar_tipo_usuario(['candidato'])
def selecao_cursos():
    resultado = banco.execute_query("SELECT data_nasc FROM candidatos WHERE id = %s", session.get('id'))

    aniversario_24 = None
    idade = 0

    if resultado:
        data_nasc = descriptografar(resultado[0]['data_nasc'])
        
        if data_nasc:
            idade = calcular_idade(data_nasc)
            
            try:
                if '-' in data_nasc and len(data_nasc.split('-')[0]) == 2:
                    nascimento = datetime.strptime(data_nasc, '%d-%m-%Y').date()
                else:
                    nascimento = datetime.strptime(data_nasc, '%Y-%m-%d').date()
                
                try:
                    aniversario_24 = nascimento.replace(year=nascimento.year + 24)
                except ValueError:
                    aniversario_24 = nascimento.replace(year=nascimento.year + 24, day=28)
    ...
    AND id IN (SELECT DISTINCT id_curso FROM vagas WHERE status = 'Ativo')
    AND id NOT IN (SELECT DISTINCT id_curso FROM cursos_concluidos WHERE id_candidato = %s)
    AND NOT EXISTS (SELECT 1 FROM candidatos WHERE id = %s AND (periodo_estudo = horario OR (periodo_estudo = 'Integral' AND horario IN ('Manhã', 'Tarde'))))""",session.get('id'), session.get('id'))

    cursos = []
    if dados:
        lista = dados if isinstance(dados, list) else [dados]
        for i in lista:
            if int(i['duracao']) < meses_faltantes and idade >= int(i['idade_minima']):
                curso = {
                    'id': i['id'],
                    'nome': f"{i['nome']} ({i['sigla']})",
                    'horario': i['horario'],
...
@candidato_bp.route('/selecionar-curso/<id_primeira_opcao>/<id_segunda_opcao>/<id_terceira_opcao>')
# @verificar_fase(['Candidatura'])
# @verificar_login(requer_login=False)
# @verificar_tipo_usuario(['candidato'])
