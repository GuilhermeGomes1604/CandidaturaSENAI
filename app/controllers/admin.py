...
def admins():
    ...
    for i in lista:
        ...
        admins.append(admin)
    return render_template('admins/admins.html', dados = dados)
...
@admin_bp.route('/edicao-admin/<int:id>')
# @verificar_login(requer_login=True)
# @verificar_tipo_usuario(['admin'])
def edicao_admin(id):
    dados = banco.execute_query("SELECT * FROM admins WHERE id = %s", id )[0]
...
@admin_bp.route('/alterar-fase')
# @verificar_login(requer_login=True)
# @verificar_tipo_usuario(['admin'])
def alterar_fase(fase):
    if request.method == 'GET':
        ...
        if fase == "Divulgação":
            ...
            for i in lista:
                candidatos = []
                for i in lista:
                    dados_candidatos = banco.execute_query("SELECT * FROM candidatos WHERE id IN (SELECT id_candidato FROM recrutamento WHERE id_empresa = %s)", i['id'])
                ...
                banco.execute_non_query("INSERT INTO (nome, id_empresa) VALUES (%s, %s)",f"Contratações - {descriptografar(i['nome_fantasia'])}", i['id'])
...
def relatorios():
    ...
    for i in lista:
        relatorio = {
            'id': i['id'],
            'nome': descriptografar(i['nome']),
            'data_registro': descriptografar(i['data_registro']),
        }
...
def cadastrar_curso():
    ...
    idade_minima = request.form.get('idade_minima', '').strip()
...
@admin_bp.route('/edicao-curso/<int:id>')
# @verificar_login(requer_login=True)
# @verificar_tipo_usuario(['admin'])
def edicao_curso(id):
...
@admin_bp.route('/edicao-candidato/<int:id>')
# @verificar_login(requer_login=True)
# @verificar_tipo_usuario(['admin'])
...
@admin_bp.route('/edicao-empresa/<int:id>')
# @verificar_login(requer_login=True)
# @verificar_tipo_usuario(['admin'])
...
@admin_bp.route('/restaurar-senha/<int:id>/<login>/<senha>')
# @verificar_login(requer_login=True)
# @verificar_tipo_usuario(['admin'])
