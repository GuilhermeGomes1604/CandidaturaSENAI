...
from app.models import Admin, Candidato, Empresa, Email, Telefone
...
def index():
    ...
    if session.get('tipo') == 'candidato':
        ...
        if dados_recrutamento:
            ...
            selecao = False
    return render_template('index.html', selecionou_cursos = selecionou_cursos, selecao = selecao, empresa = empresa)

@geral_bp.route('/reports/<path:filename>')
    ...
    if request.method == 'GET':
        ...
        if len(login) == 14 :
            logins = banco.execute_query("""
                SELECT id, nome, cpf, senha, 'admin' AS tipo, status FROM admins 
                UNION 
                SELECT id, nome, cpf, senha, 'candidato' AS tipo, status FROM candidatos
            """)
                ...
                for i in lista:
                    if descriptografar(i['cpf']) == login:
                        if check_password_hash(i['senha'], senha):                              
                            ...
                            if i['status'] == 'Inativo':
                                tabela = f"{i['tipo']}" + "s"
                                banco.execute_non_query("UPDATE %s SET status = 'Ativo' WHERE id = %s", tabela, i['id'])
            ...
            logins = banco.execute_query("""
                SELECT id, nome_fantasia AS nome, cnpj, senha, 'empresa' AS tipo, status FROM empresas
            """)
                ...
                for i in lista:
                    if descriptografar(i['cnpj']) == login:
                        if check_password_hash(i['senha'], senha):
                            ...
                            if i['status'] == 'Inativo':
                                banco.execute_non_query("UPDATE empresas SET status = 'Ativo' WHERE id = %s", i['id'])
                                banco.execute_non_query("UPDATE vagas SET status = 'Ativo' WHERE id_empresa = %s", i['id'])
...
@geral_bp.route('/esqueci-minha-senha', methods=['GET', 'POST'])
# @verificar_login(requer_login=False)
def esqueci_senha():
    if request.method == 'GET':
        ...
        if len(login) == 14 and not validar_documento(login, 'cpf'):
            flash("CPF Inválido!","erro")
            return redirect(url_for('geral.esqueci_senha'))
        elif len(login) == 18 and not validar_documento(login, 'cnpj'):
            flash("CNPJ Inválido!","erro")
            return redirect(url_for('geral.esqueci_senha'))

        logins = banco.execute_query("""
            SELECT id, nome, cpf AS documento, senha, 'admin' AS tipo FROM admins 
            UNION 
            SELECT id, nome, cpf AS documento, senha, 'candidato' AS tipo FROM candidatos
            UNION
            SELECT id, nome_fantasia AS nome, cnpj AS documento, senha, 'empresa' AS tipo FROM empresas
        """)
        ...
        if logins:
            ...
            for i in lista:
                if descriptografar(i['documento']) == login and descriptografar(i['nome']).lower() == nome.lower():
                    banco.execute_non_query("INSERT INTO requisicoes_mudanca_senha (nome, login) VALUES (%s, %s)", criptografar(nome), criptografar(login))
                    flash("Requisição de alteração de senha feita com sucesso! Contate a administração do SENAI para definir uma nova senha","info")
                    return redirect(url_for('geral.login'))                
    flash("CPF/CNPJ ou nome não encontrado/não correspondentes!","erro")
    return redirect(url_for('geral.esqueci_senha'))

@geral_bp.route('/usuario', methods=['GET','POST'])
# @verificar_login(requer_login=True)
def usuario():
...
def edicao_dados():
    ...
    return render_template('editar-dados.html', dados = dados, emails = emails, telefones = telefones)
    ...
    match session.get('tipo'):
        case 'admin':
            ...
            if not validar_documento(cpf, 'cpf'):
                flash("CPF inválido!", "erro")
                return redirect(url_for('geral.edicao_dados'))
            ...
            if cpfs:
                ...
                for i in lista:
                    if descriptografar(i['cpf']) == cpf and i['id'] != session.get('id'):
        ...
        case 'empresa':
            ...
            if cnpjs:
                ...
                for i in lista:
                    if descriptografar(i['cnpj']) == cnpj and i['id'] != session.get('id'):
...
def api_dados_cursos():
    ...
    dados = banco.execute_query(query, *parametros)
    cursos = []
    if dados:
...
def api_dados_candidatos():
    ...
    horarios = [h for h in request.args.getlist('horarios') if h]
    cursos = [c for c in request.args.getlist('cursos') if c]
    tipo_cursos = [t for t in request.args.getlist('tipo-cursos') if t]
    ...
    if horarios:
        placeholders = ', '.join(['%s'] * len(horarios))
        query += f" AND periodo_estudo NOT IN ({placeholders})"
        parametros.extend(horarios)
    ...
    if session.get('tipo') == 'admin' and cursos:
        ...
        if resultado_cursos:
            cursos_encontrados = resultado_cursos if isinstance(resultado_cursos, list) else [resultado_cursos]
            ids = [c['id'] if isinstance(c, dict) else c[0] for c in cursos_encontrados]
    ...
    elif session.get('tipo') == 'empresa' and tipo_cursos and 'vagas' in tipo_cursos:
        cursos_vagas = banco.execute_query(
            "SELECT DISTINCT id_curso FROM vagas WHERE id_empresa = %s AND id_curso IS NOT NULL", 
            session.get('id')
        )

        if cursos_vagas:
            lista_vagas = cursos_vagas if isinstance(cursos_vagas, list) else [cursos_vagas]
            ids_vagas = [v['id_curso'] if isinstance(v, dict) else v[0] for v in lista_vagas]

            if ids_vagas:
                placeholders_ids = ', '.join(['%s'] * len(ids_vagas))
                query += f""" AND (id_primeira_opcao IN ({placeholders_ids})
                OR id_segunda_opcao IN ({placeholders_ids})
                OR id_terceira_opcao IN ({placeholders_ids}))"""
                parametros.extend(ids_vagas * 3)
            else:
                query += " AND 0 = 1"
        else:
            query += " AND 0 = 1"

    dados = banco.execute_query(query, *parametros) if parametros else banco.execute_query(query)
    candidatos = []
    
    if dados:
        lista = dados if isinstance(dados, list) else [dados]
        for i in lista:
            nome = descriptografar(i.get('nome')) if i.get('nome') else ""
            nome_social = descriptografar(i.get('nome_social')) if i.get('nome_social') else ""
            
            if pesquisa:
                corresponde = True
                for palavra in pesquisa:
                    palavra_minuscula = palavra.lower()
                    if (palavra_minuscula not in nome.lower()) and (palavra_minuscula not in nome_social.lower()):
                        corresponde = False
                        break
            ...
            if i.get('periodo_estudo'):
                if i.get('periodo_estudo') == "Integral":
                    periodo_livre = ["Noite"]
                elif i.get('periodo_estudo') in periodo_livre:
                    periodo_livre.remove(i.get('periodo_estudo'))
            
            relacao = None
            if session.get('tipo') == 'empresa':
                relacao_dados = banco.execute_query(
                    "SELECT relacao FROM recrutamento WHERE id_candidato = %s AND id_empresa = %s", 
                    i.get('id'), session.get('id')
                )
                if relacao_dados:
                    primeiro = relacao_dados[0] if isinstance(relacao_dados, list) else relacao_dados
                    relacao = primeiro['relacao'] if isinstance(primeiro, dict) else primeiro[0]

            candidato = {
                'id': i.get('id'),
                'nome': nome_social if nome_social else nome,
                'idade': calcular_idade(descriptografar(i.get('data_nasc'))) if i.get('data_nasc') else "",
                'escolaridade': i.get('escolaridade', ''),
                'periodo_livre': periodo_livre,
                'ja_estudou': i.get('ja_estudou', ''),
                'id_primeira_opcao': i.get('id_primeira_opcao'),
                'id_segunda_opcao': i.get('id_segunda_opcao'),
                'id_terceira_opcao': i.get('id_terceira_opcao'),
                'disponibilidade': i.get('disponibilidade', ''),
                'relacao': relacao,
                'status': i.get('status', 'Ativo'),
                'data_registro': formatar_data_e_hora(i.get('data_registro')) if i.get('data_registro') else ""
            }
...
def candidato(id):
    ...
    if dados:
        lista = dados if isinstance(dados, list) else [dados]
        for i in lista:
            dados_recrutamento = banco.execute_query("SELECT * FROM recrutamento WHERE id_candidato = %s", id)
            if dados_recrutamento:
                id_empresa = dados_recrutamento[0]['id_empresa']
                empresa = banco.execute_query("SELECT nome_fantasia FROM empresas WHERE id = %s",id_empresa)[0]['nome_fantasia']
                relacao = dados_recrutamento[0]['relacao']
                ...
                candidato = {
                    ...
                    'ja_estudou': dados['ja_estudou'],
                    'disponibilidade': dados['disponibilidade'],
                    'empresa': descriptografar(empresa),
                    'relacao': relacao,
                    'data_registro': formatar_data_e_hora(dados['data_registro'])
                }
...
def vagas(id):
    ...
    empresa = {
        'id': id,
        'nome': descriptografar(dados['nome_fantasia'])
    }
...
@geral_bp.route('/api/dados-vagas/<int:id>')
# @verificar_login(requer_login=True)
# @verificar_tipo_usuario('admin','empresa')
def api_dados_vagas(id):
    dados = banco.execute_query("SELECT * FROM vagas WHERE id_empresa = %s AND status = 'Ativo'", id)
    vagas = []
    if dados:
        lista = dados if isinstance(dados, list) else [dados]
        for i in lista:
            dados_curso = banco.execute_query("SELECT * FROM cursos WHERE id = %s", i['id_curso'])
            if dados_curso:
                curso = dados_curso[0] if isinstance(dados_curso, list) else dados_curso
                vaga = {
                    'id': i['id'],
                    'curso': f"{curso['nome']} ({curso['sigla']})",
...
def marcados(id):
    resultado_empresa = banco.execute_query("SELECT nome_fantasia FROM empresas WHERE id = %s", id)
    empresa = resultado_empresa[0]['nome_fantasia'] if isinstance(resultado_empresa[0], dict) else resultado_empresa[0][0]
    ...
    candidatos = [] # Corrigido: Inicialização da lista que faltava aqui
        ...
        for i in lista:
            ...
            if i['periodo_estudo']:
                if i['periodo_estudo'] == "Integral":
                    periodo_livre = ["Noite"]
                elif i['periodo_estudo'] in periodo_livre:
                    periodo_livre.remove(i['periodo_estudo'])

            relacao_dados = banco.execute_query("SELECT relacao FROM recrutamento WHERE id_candidato = %s AND id_empresa = %s", i['id'], id)
            if relacao_dados:
                primeiro = relacao_dados[0] if isinstance(relacao_dados, list) else relacao_dados
                relacao = primeiro['relacao'] if isinstance(primeiro, dict) else primeiro[0]
            else:
            ...
            candidato = {
                'id': i['id'],
                'nome': nome_social if nome_social else nome,
                'idade': calcular_idade(descriptografar(i['data_nasc'])) if i['data_nasc'] else "",
                'escolaridade': i['escolaridade'] if i['escolaridade'] else '',
                'periodo_livre': periodo_livre,
                'ja_estudou': i['ja_estudou'] if i['ja_estudou'] else '',
                'id_primeira_opcao': i['id_primeira_opcao'],
                'id_segunda_opcao': i['id_segunda_opcao'],
                'id_terceira_opcao': i['id_terceira_opcao'],
                'disponibilidade': i['disponibilidade'] if i['disponibilidade'] else '',
                'relacao': relacao
            }
            candidatos.append(candidato)
            
    return render_template('marcados.html', candidatos=candidatos, empresa=empresa)
...
def selecionados(id):
    ...
    return render_template('selecionados.html', candidatos=candidatos, empresa=empresa)

@geral_bp.route('/exibir-imagem-usuario')
def exibir_imagem_usuario():
    id = session.get('id')
    match session.get('tipo'):
        case 'admin':
...
@geral_bp.route('/exibir-imagem-admin/<int:id>')
def exibir_imagem_admin(id):
    nome_arquivo = f'admin_{id}.jpg'
    pasta = os.path.join(current_app.config['UPLOAD_FOLDER'], 'admins')
